...
## Installation
Clone this repo then run the ./mkbinlink scrpt to create symlinks in `~/bin` to the executable scripts in this repo.

## Usage
How the scrpts are used varies depending on the directory they are found in.

* Root of repo: These are all top-level executable scrpts. After completing the inbstallation steps described above, you can run them by just specifying the filename without the .sh extension (as they appear in `~/bin`). These should all be relatively complete and mature top-level scripts with their own help (-h) and everything. So approach them as any other software you install via packages.
...
```
for awesomeShellScript in ~/$BEST_REPO_EVER/env/*; do
